...
public class DialogueManager extends BaseManager<Dialogue> {
   ...
   protected Dialogue createData(String id, class_2487 tag) {
   ...
   public void open(class_3222 player, Dialogue dialogue, DialogueContext context) {
      ICharacter character = Character.get(player);
      if (character != null) {
         character.setDialogue(dialogue, context);
         Mappet.dialogues.execute(dialogue, context);
         if (context.reactionNode != null) {
            this.handleContext(player, dialogue, context, (ReactionNode)null);
         }
      }

   }

   public void handleContext(class_3222 player, Dialogue dialogue, DialogueContext context, ReactionNode last) {
      if (last != null && !last.sound.isEmpty()) {
         WorldUtils.stopSound(player, last.sound);
      }

      List<DialogueFragment> replies = (List)context.replyNodes.stream().map((r) -> r.message.copy().process(context.data)).collect(Collectors.toList());
