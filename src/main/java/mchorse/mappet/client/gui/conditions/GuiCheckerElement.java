...
public class GuiCheckerElement extends GuiElement {
   ...
   public GuiCheckerElement(class_310 mc) {
      this(mc, (Checker)null);
   }

   public GuiCheckerElement(class_310 mc, Checker checker) {
      super(mc);
      this.expression = GuiMappetUtils.fullWindowContext(new GuiTextElement(mc, 10000, (text) -> this.checker.expression = text), IKey.lang("mappet.gui.checker.expression"));
   ...
   private void updateFields() {
      this.removeAll();
      this.add(new IGuiElement[]{(IGuiElement)(this.checker.mode == Checker.Mode.CONDITION ? this.condition : this.expression), this.toggle});
