...
public class GuiWorldTimeConditionBlockPanel extends GuiAbstractConditionBlockPanel<WorldTimeConditionBlock> {
   ...
   public GuiWorldTimeConditionBlockPanel(class_310 mc, GuiConditionOverlayPanel overlay, WorldTimeConditionBlock block) {
      ...
      this.min.limit((double)0.0F, (double)24000.0F, true).setValue((double)block.min);
      this.max = new GuiTrackpadElement(mc, (v) -> (this.block).max = v.intValue());
      this.max.limit((double)0.0F, (double)24000.0F, true).setValue((double)block.max);
      GuiElement a = Elements.label(IKey.lang("mappet.gui.conditions.world_time.range")).marginTop(12);
      GuiElement b = Elements.row(mc, 5, new GuiElement[]{this.min, this.max});
      this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.conditions.world_time.check")).marginTop(12), this.type});
      this.add(new IGuiElement[]{a, b});
