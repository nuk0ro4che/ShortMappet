...
public class GuiFactions extends GuiElement {
   ...
   public GuiFactions(class_310 mc) {
      super(mc);
      this.flex().column(5).stretch().vertical();
   }

   public void addRelation(String faction, FactionAttitude attitude, boolean put) {
      GuiCirculateElement button = GuiFactionPanel.createButton(this.mc, (a) -> this.relations.put(faction, a));
      GuiElement row = Elements.row(this.mc, 5, new GuiElement[]{Elements.label(IKey.str(faction), 20).anchor(0.0F, 0.5F), button});
   ...
   public void set(Map<String, FactionAttitude> relations) {
      ...
      for(String key : relations.keySet()) {
         this.addRelation(key, (FactionAttitude)relations.get(key), false);
      }
