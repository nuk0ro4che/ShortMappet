...
public class GuiCommandNodePanel extends GuiEventBaseNodePanel<CommandNode> {
   ...
   public GuiCommandNodePanel(class_310 mc) {
      super(mc);
      this.command = GuiMappetUtils.fullWindowContext(new GuiTextElement(mc, 10000, (text) -> (this.node).command = text), IKey.lang("mappet.gui.nodes.event.command"));
      this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.nodes.event.command")).marginTop(12), this.command, this.binary});
   }
