...
public class GuiTriggerNodePanel extends GuiEventBaseNodePanel<TriggerNode> {
   ...
   public GuiTriggerNodePanel(class_310 mc) {
      super(mc);
      this.trigger = new GuiTriggerElement(mc);
      this.customData = GuiMappetUtils.fullWindowContext(new GuiTextElement(mc, 100000, (text) -> (this.node).customData = text), IKey.lang("mappet.gui.nodes.event.data"));
      ...
      this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.nodes.event.data")).marginTop(12), this.customData, this.cancel, this.binary});
   }
