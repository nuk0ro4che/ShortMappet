...
public class GuiNpcMovementPanel extends GuiNpcPanel {
   ...
   public GuiNpcMovementPanel(class_310 mc) {
      super(mc);
      ...
      addSteeringOffset.flex().relative(steeringOffsetLabel).xy(1.0F, 0.5F).w(10).anchor(1.0F, 0.5F);
      ...
      addPatrol.flex().relative(patrolLabel).xy(1.0F, 0.5F).w(10).anchor(1.0F, 0.5F);
      patrolLabel.add(addPatrol);
      this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.npcs.movement.speed")), this.speed});
      this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.npcs.movement.flight_max_height")), this.flightMaxHeight});
      this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.npcs.movement.flight_min_height")), this.flightMinHeight});
      this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.npcs.movement.jump_power")), this.jumpPower});
      ...
      this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.npcs.movement.follow")).marginTop(12), this.follow});
   ...
   public void set(NpcState state) {
      super.set(state);
      this.speed.setValue((double)(Float)state.speed.get());
      this.flightMaxHeight.setValue((Double)state.flightMaxHeight.get());
      this.flightMinHeight.setValue((Double)state.flightMinHeight.get());
      this.jumpPower.setValue((double)(Float)state.jumpPower.get());
      this.steeringOffset.set(state.steeringOffset);
      this.canSwim.toggled((Boolean)state.canSwim.get());
      this.immovable.toggled((Boolean)state.immovable.get());
      this.hasPost.toggled((Boolean)state.hasPost.get());
      this.postPosition.set(state.postPosition);
      this.postRadius.setValue((double)(Float)state.postRadius.get());
      this.patrolCirculate.toggled((Boolean)state.patrolCirculate.get());
