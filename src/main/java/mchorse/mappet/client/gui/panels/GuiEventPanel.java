...
public class GuiEventPanel extends GuiMappetRunPanel<NodeSystem<EventBaseNode>> {
   ...
   public static final Map<Class<? extends EventBaseNode>, Class<? extends GuiEventBaseNodePanel<? extends EventBaseNode>>> PANELS = new HashMap();
   public GuiEventNodeGraph graph;
   public GuiEventBaseNodePanel panel;

   public GuiEventPanel(class_310 mc, GuiMappetDashboard dashboard) {
      super(mc, dashboard);
   ...
   private void pickNode(EventBaseNode node) {
      ...
      if (node != null) {
         ...
         try {
            panel = (GuiEventBaseNodePanel)((Class)PANELS.get(node.getClass())).getConstructor(class_310.class).newInstance(this.mc);
            panel.set(node);
         } catch (Exception e) {
            e.printStackTrace();
         }
   ...
   protected void run(class_746 player) {
      ...
      var10000.method_45731("mp event trigger " + var10001 + " " + ((NodeSystem)this.data).getId());
   }
   ...
   public void draw(GuiContext context) {
      ...
      if (!this.graph.isVisible()) {
         ...
         GuiDraw.drawMultiText(this.font, EMPTY_EVENT.get(), x, this.area.my(), 16777215, w, 12, 0.5F, 0.5F);
      }
