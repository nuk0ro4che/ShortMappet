...
public class GuiFactionPanel extends GuiMappetDashboardPanel<Faction> {
   ...
   public static GuiCirculateElement createButton(class_310 mc, Consumer<FactionAttitude> callback) {
   ...
   public GuiFactionPanel(class_310 mc, GuiMappetDashboard dashboard) {
      super(mc, dashboard);
      ...
      this.color = new GuiColorElement(mc, (cx) -> (this.data).color = cx);
      this.score = new GuiTrackpadElement(mc, (v) -> (this.data).score = v.intValue());
      this.score.limit((double)0.0F).integer();
      this.playerAttitude = createButton(mc, (ax) -> (this.data).playerAttitude = ax);
      this.othersAttitude = createButton(mc, (ax) -> (this.data).othersAttitude = ax);
      this.openOwnRelation = new GuiButtonElement(mc, IKey.lang("mappet.gui.factions.relations.open"), (bx) -> this.openRelation());
      this.relations = new GuiFactions(mc);
      GuiElement a = new GuiElement(mc);
      a.flex().column(4).vertical().stretch();
      a.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.factions.title")), this.title});
      GuiElement b = new GuiElement(mc);
      b.flex().w(140).column(4).vertical().stretch();
      b.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.factions.color")), this.color});
      GuiElement c = new GuiElement(mc);
      c.flex().column(4).vertical().stretch();
      c.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.factions.others_attitude")), this.othersAttitude});
      GuiElement d = new GuiElement(mc);
      d.flex().column(4).vertical().stretch();
      d.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.factions.player_attitude")), this.playerAttitude});
      ...
      GuiIconElement add = new GuiIconElement(mc, Icons.ADD, (button) -> {
         List<String> keys = new ArrayList(this.names.list.getList());
      ...
      add.flex().relative(label).xy(1.0F, 0.5F).w(10).anchor(1.0F, 0.5F);
      label.add(add);
      GuiScrollElement scrollEditor = this.createScrollEditor();
      scrollEditor.add(Elements.row(mc, 5, new GuiElement[]{a, b}));
      scrollEditor.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.factions.visible")).marginTop(12), this.visible});
      scrollEditor.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.factions.score")).marginTop(12), this.score});
      scrollEditor.add(Elements.row(mc, 5, new GuiElement[]{c, d}).marginTop(12));
