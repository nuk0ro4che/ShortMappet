...
public class GuiObjectives extends GuiElement {
   ...
   public GuiObjectives(class_310 mc) {
      super(mc);
      this.flex().column(20).vertical().stretch();
   }

   public GuiSimpleContextMenu getAdds() {
      return (new GuiSimpleContextMenu(class_310.method_1551())).action(Icons.ADD, IKey.lang("mappet.gui.quests.objectives.context.add_kill"), () -> this.addObjective(new KillObjective(), true)).action(Icons.ADD, IKey.lang("mappet.gui.quests.objectives.context.add_collect"), () -> this.addObjective(new CollectObjective(), true)).action(Icons.ADD, IKey.lang("mappet.gui.quests.objectives.context.add_state"), () -> this.addObjective(new StateObjective(), true)).action(Icons.ADD, IKey.lang("mappet.gui.quests.objectives.context.add_dialogue_read"), () -> this.addObjective(this.createDialogueReadObjective(), true));
   }
   ...
   private AbstractObjective createDialogueReadObjective() {
      ...
      objective.message = class_1074.method_4662("mappet.gui.quests.objective_state.dialogue", new Object[0]);
      return objective;
   }
   ...
   private void addObjective(AbstractObjective objective, boolean add) {
      ...
      if (element != null) {
         GuiObjective finalElement = element;

         this.add(element);
         element.context(() -> (new GuiSimpleContextMenu(class_310.method_1551()))
                 .action(Icons.REMOVE,
                         IKey.lang("mappet.gui.quests.objectives.context.remove"),
                         () -> this.removeObjective(finalElement), 
                         16711731));
         if (add) {
