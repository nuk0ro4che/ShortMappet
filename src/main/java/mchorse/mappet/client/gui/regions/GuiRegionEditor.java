...
public class GuiRegionEditor extends GuiElement {
   ...
   public GuiRegionEditor(class_310 mc) {
      super(mc);
      ...
      this.delay = (new GuiTrackpadElement(mc, (value) -> this.region.delay = value.intValue())).limit((double)0.0F).integer();
      this.update = (new GuiTrackpadElement(mc, (value) -> this.region.update = value.intValue())).limit((double)1.0F).integer();
      ...
      this.stateOptions = Elements.column(mc, 5, new GuiElement[0]);
      this.state = new GuiTextElement(mc, (t) -> this.region.state = t);
      this.target = GuiMappetUtils.createTargetCirculate(mc, TargetMode.GLOBAL, (targetx) -> this.region.target = targetx);
      ...
      this.shapes = Elements.column(mc, 5, new GuiElement[0]);
      this.add(this.passable);
      this.add(this.checkEntities);
      this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.region.enabled")).marginTop(6), this.enabled});
      this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.region.delay")).marginTop(12), this.delay});
      this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.region.update")).marginTop(12), this.update});
      this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.region.on_enter")).background().marginTop(12).marginBottom(5), this.onEnter});
      this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.region.on_exit")).background().marginTop(12).marginBottom(5), this.onExit});
      this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.region.on_tick")).background().marginTop(12).marginBottom(5), this.onTick});
      ...
      addShape.flex().relative(shapesLabel).xy(1.0F, 0.5F).w(10).anchor(1.0F, 0.5F);
   ...
   public void set(Region region) {
      ...
      if (region != null) {
         ...
         this.delay.setValue((double)region.delay);
         this.update.setValue((double)region.update);
