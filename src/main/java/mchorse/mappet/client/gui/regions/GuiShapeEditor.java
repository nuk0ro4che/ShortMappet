...
public class GuiShapeEditor extends GuiElement {
   ...
   public GuiShapeEditor(class_310 mc) {
      super(mc);
      this.context(() -> (new GuiSimpleContextMenu(this.mc)).action(Icons.REMOVE, IKey.lang("mappet.gui.region.context.remove"), this::removeShape, 16711731));
      ...
      this.bottomRow = Elements.row(mc, 5, 0, new GuiElement[]{this.sizeX, this.sizeY, this.sizeZ});
      this.add(this.shapeSwitch);
      this.add(Elements.label(IKey.lang("mappet.gui.region.offset")));
      this.add(Elements.row(mc, 5, 0, new GuiElement[]{this.x, this.y, this.z}));
      this.add(new IGuiElement[]{this.bottomLabel, this.bottomRow});
   }
