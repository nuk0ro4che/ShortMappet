...
public abstract class GuiDataTriggerBlockPanel<T extends DataTriggerBlock> extends GuiStringTriggerBlockPanel<T> {
   ...
   public GuiDataTriggerBlockPanel(class_310 mc, GuiTriggerOverlayPanel overlay, T block) {
      super(mc, overlay, block);
      this.data = GuiMappetUtils.fullWindowContext(new GuiTextElement(mc, 100000, (text) -> (this.block).customData = text), IKey.lang("mappet.gui.nodes.event.data"));
