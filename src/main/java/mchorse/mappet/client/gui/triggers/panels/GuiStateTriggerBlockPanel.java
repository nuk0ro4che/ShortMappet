...
public class GuiStateTriggerBlockPanel extends GuiAbstractTriggerBlockPanel<StateTriggerBlock> {
   ...
   public GuiStateTriggerBlockPanel(class_310 mc, GuiTriggerOverlayPanel overlay, StateTriggerBlock block) {
      super(mc, overlay, block);
      this.id = new GuiTextElement(mc, 1000, (v) -> (this.block).string = v);
      this.target = new GuiTargetElement(mc, (Target)null);
      ...
      this.valueRow = Elements.row(mc, 0, new GuiElement[0]);
      ...
      this.add(this.mode);
      this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.conditions.state.id")).marginTop(12), this.id});
      this.add(this.target.marginTop(12));
      this.add(new IGuiElement[]{this.valueLabel.marginTop(12), this.valueRow});
   ...
   private void updateValue() {
      ...
      if (object instanceof String) {
         if ((this.block).mode == StateTriggerBlock.StateMode.ADD) {
            (this.block).value = object = (double)0.0F;
         } else {
