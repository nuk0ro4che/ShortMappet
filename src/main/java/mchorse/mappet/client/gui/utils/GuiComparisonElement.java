...
public class GuiComparisonElement extends GuiElement {
   ...
   public GuiComparisonElement(class_310 mc, Comparison comparison) {
      super(mc);
   ...
   private void toggleComparison(GuiCirculateElement b) {
      ...
      this.removeAll();
      this.add(Elements.column(this.mc, 5, new GuiElement[]{Elements.label(IKey.lang("mappet.gui.conditions.comparison")), this.mode}));
      if (insert != null) {
         this.add(Elements.column(this.mc, 5, new GuiElement[]{Elements.label(label), insert}));
      }
