...
public class GuiGlobalMigrateConfirmModal extends GuiModal {
   ...
   private final GuiButtonElement copy;
   private final GuiButtonElement cancel;
   private final Consumer<Boolean> callback;

   public GuiGlobalMigrateConfirmModal(class_310 mc, IKey label, Consumer<Boolean> callback) {
      super(mc, label);
      this.callback = callback;
      this.card = new GuiElement(mc);
      this.card.flex().relative(this).xy(0.5F, 0.5F).wh(380, 170).anchor(0.5F, 0.5F);
      this.remove(this.bar);
      this.card.add(this.bar);
      this.add(this.card);
      this.copy = new GuiButtonElement(mc, IKey.lang("mappet.config.general.global_migrate_button.action"), (button) -> this.close(true));
      this.cancel = new GuiButtonElement(mc, IKey.lang("mappet.gui.cancel"), (button) -> this.close(false));
      this.copy.color(ACCEPT).textColor(16777215, true);
      this.cancel.color(CANCEL).textColor(16777215, true);
      this.bar.flex().relative(this.card).y(1.0F).w(1.0F).h(30).anchorY(1.0F).row(8).padding(10);
      this.bar.add(this.copy, this.cancel);
   }
   ...
   public void draw(GuiContext context) {
      ...
      for(String line : lines) {
         GuiDraw.drawStringWithShadow(this.font, line, this.card.area.x + 14, y, 16777215);
         y += 12;
      }
