...
public class GuiTargetElement extends GuiElement {
   ...
   public GuiTargetElement(class_310 mc, Target target) {
      super(mc);
   ...
   private void updateTarget() {
      if (this.target != null) {
         this.removeAll();
         this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.conditions.target")), this.mode});
         if (this.target.mode == TargetMode.SELECTOR) {
            this.add(new IGuiElement[]{Elements.label(IKey.lang("mappet.gui.conditions.selector")), this.selector});
         }
